...
import { ClaudeRateLimitError, FixedWindowRateLimiter } from "../src/services/rateLimiters.js";
  ...
  return runSuite("moodEngine", [
    ...
    },
    {
      ...
      run: async () => {
        ...
        const reserveClaudeCapacity = (requesterId: string, now = Date.now()) => {
        ...
        await assert.rejects(
          ...
          (error: unknown) => error instanceof ClaudeRateLimitError
        );
      },
    },
  ]);
